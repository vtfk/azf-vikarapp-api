...
async function searchUsersInGroup (searchTerm, groupId, requestor, returnSelf) {
  ...
  // Prepare the request
  const request = {
    url: `https://graph.microsoft.com/v1.0/groups/${groupId}/members?$search="displayName:${searchTerm}"&$select=id,displayName,jobTitle,officeLocation,userPrincipalName,companyName`,
    metod: 'get',
    headers: {
