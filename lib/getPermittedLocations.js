...
async function getPermittedLocations (requestor) {
  ...
  if ((!requestor.roles || !requestor.roles.includes('App.Admin')) && requestor.officeLocation) {
    ...
    await connect()
    const school = await Schools.findOne({ name: requestor.officeLocation }).populate('permittedSchools', '_id name').lean()
